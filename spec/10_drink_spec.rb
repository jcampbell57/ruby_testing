...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:new_drink) { described_class.new('water', 16) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(new_drink.type).to eql('water')
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:water_drink) { described_class.new('water', 16) }

      # remove the 'x' before running this test
      it 'is full' do
        expect(water_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:tea_drink) { described_class.new('tea', 8) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(tea_drink).not_to be_full
      end
    end
  end
end
