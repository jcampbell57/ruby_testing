...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min and max both equal 3' do
      ...
      it 'returns 3' do
        guess = game_guessing5.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:winning_number) { double('random_number', value: 4) }
      subject(:winning_game) { described_class.new(0, 9, winning_number, 4) }
    ...
    context 'when guess and random_number are not equal' do
      let(:winning_number) { double('random_number', value: 4) }
      subject(:winning_game) { described_class.new(0, 9, winning_number, 8) }
      it 'is not game over' do
        expect(winning_game).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'does not update max' do
        old_max = edge_game.max
        edge_game.update_range
        expect(edge_game.max).to eq(old_max)
      end
    end
  end
end
