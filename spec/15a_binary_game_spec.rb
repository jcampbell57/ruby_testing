...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    subject(:test_game) { described_class.new(1, 10) }

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = test_game.instance_variable_get(:@minimum)
        max = test_game.instance_variable_get(:@maximum)
        result = test_game.verify_input(min, max, 4)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        min = test_game.instance_variable_get(:@minimum)
        max = test_game.instance_variable_get(:@maximum)
        result = test_game.verify_input(min, max, 14)
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(binary_search) }.to change {
                                                                        game_turn_order.instance_variable_get(:@guess_count)
                                                                      }.by(1)
    end
